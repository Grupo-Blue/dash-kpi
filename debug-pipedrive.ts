...
async function debugPipedrive() {
  ...
  try {
    ...
    const allPipelines = await service.getAllPipelines();
    ...
    const salesDeals = await service.getDeals({ pipeline_id: salesPipeline.id });
    ...
    const implDeals = await service.getDeals({ pipeline_id: implPipeline.id });
    ...
    const salesStages = await service.getStages(salesPipeline.id);
    const implStages = await service.getStages(implPipeline.id);
