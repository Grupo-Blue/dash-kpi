...
class LeadJourneyService {
  ...
  private analyzeAcquisition(mauticData: MauticLeadJourneyData): Record<string, any> | null {
    ...
    let query = (firstPageHit as any)?.details?.query || (firstPageHit as any)?.details?.properties?.query;

    // P1-4: Tratar query quando vier como string (comum no Mautic)
  ...
  private buildLeadJourneyData(
    ...
    // Guard clause: verificar se contact existe
    if (!mauticData.contact) {
      throw new Error('[LeadJourney] Contact data is null - cannot build journey data');
    }
